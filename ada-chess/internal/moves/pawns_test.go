...
func TestWhitePawnAttacks_Center(t *testing.T) {
	// e4: attacks d5, f5
	got := PawnAttacks(sq(3, 4), board.White)
...
func TestBlackPawnAttacks_Center(t *testing.T) {
	// e5: attacks d4, f4
	got := PawnAttacks(sq(4, 4), board.Black)
...
func TestWhitePawnAttacks_AFile(t *testing.T) {
	...
	got := PawnAttacks(sq(1, 0), board.White)
	want := bb(sq(2, 1))
	if got != want {
...
func TestWhitePawnAttacks_HFile(t *testing.T) {
	...
	got := PawnAttacks(sq(3, 7), board.White)
	want := bb(sq(4, 6))
	if got != want {
...
func TestBlackPawnAttacks_AFile(t *testing.T) {
	// a7: attacks b6 only
	got := PawnAttacks(sq(6, 0), board.Black)
	want := bb(sq(5, 1))
	if got != want {
...
func TestBlackPawnAttacks_HFile(t *testing.T) {
	// h5: attacks g4 only
	got := PawnAttacks(sq(4, 7), board.Black)
	want := bb(sq(3, 6))
	if got != want {
...
func TestWhitePawnAttacks_Rank8(t *testing.T) {
	...
	for file := 0; file < 8; file++ {
		got := PawnAttacks(sq(7, file), board.White)
		if !got.Empty() {
...
func TestBlackPawnAttacks_Rank1(t *testing.T) {
	...
	for file := 0; file < 8; file++ {
		got := PawnAttacks(sq(0, file), board.Black)
		if !got.Empty() {
...
func TestPawnAttacks_AllSquaresCount(t *testing.T) {
	for s := board.Square(0); s < 64; s++ {
		...
		// white
		wc := PawnAttacks(s, board.White).Count()
		wantW := 0
		if rank < 7 {
			if file > 0 {
				wantW++
		...
		// black
		bc := PawnAttacks(s, board.Black).Count()
		wantB := 0
		if rank > 0 {
			if file > 0 {
				wantB++
...
func BenchmarkPawnAttacks(b *testing.B) {
	for i := 0; i < b.N; i++ {
		for s := board.Square(0); s < 64; s++ {
			PawnAttacks(s, board.White)
			PawnAttacks(s, board.Black)
		}
	}
}
