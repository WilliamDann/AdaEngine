...
func TestLegalMoves_DoubleCheck_OnlyKingMoves(t *testing.T) {
	...
	pos, _ := fen.Parse("4r3/8/8/8/2b5/8/3P4/3KR2k w - - 0 1")
	// Black rook e8 and bishop c4 — wait, need to construct actual double check.
	// Let's use a known double check position.
	// King d1, black rook e1... no that's check from one piece.
	// After a discovered check: bishop moves revealing rook check.
	pos, _ = fen.Parse("4k3/8/8/8/8/8/r2PK2b/8 w - - 0 1")
	// Hmm, let me use a position where both a bishop and rook give check.
	// King on e1, black bishop on b4 (checks on diagonal), black rook on e8 (checks on file)
	// but e-file has the king, so rook checks. Bishop b4 checks e1? b4=(3,1) e1=(0,4): diff 3,3 = diagonal yes.
	// But something must be between for it to be a "real" double check scenario.
	// Actually, double check arises from a discovered check. Let me just verify
	// that when in double check, only king moves are returned.
	...
	for i := 0; i < ml.Count(); i++ {
		m := ml.Get(i)
		if m.From() != sq(0, 4) { // all moves must be from e1 (king)
...
func TestLegalMoves_PinnedPieceCantMoveFreely(t *testing.T) {
	...
	for i := 0; i < ml.Count(); i++ {
		m := ml.Get(i)
		if m.From() == sq(3, 4) { // e4 = the knight
...
func TestLegalMoves_Castling(t *testing.T) {
	...
	for i := 0; i < ml.Count(); i++ {
		m := ml.Get(i)
		if m.MoveType() == board.MoveCastling {
			if m.To() == sq(0, 6) {
				foundKingside = true
			}
			if m.To() == sq(0, 2) {
...
func TestLegalMoves_CastlingBlockedByAttack(t *testing.T) {
	...
	for i := 0; i < ml.Count(); i++ {
		m := ml.Get(i)
		if m.MoveType() == board.MoveCastling && m.To() == sq(0, 6) {
...
func TestLegalMoves_Promotion(t *testing.T) {
	...
	for i := 0; i < ml.Count(); i++ {
		m := ml.Get(i)
		if m.MoveType() == board.MovePromotion {
			promoCount++
		}
	}
...
func TestLegalMoves_EnPassant(t *testing.T) {
	...
	for i := 0; i < ml.Count(); i++ {
		m := ml.Get(i)
		if m.MoveType() == board.MoveEnPassant {
			foundEP = true
			if m.From() != sq(4, 4) || m.To() != sq(5, 3) {
...
func makeMove(pos *game.Position, m board.Move) *game.Position {
	...
	case board.MovePromotion:
		newPos.Board.Set(to, board.NewPiece(m.PromoPiece(), pos.ActiveColor))
	case board.MoveEnPassant:
		...
		capturedSq := board.Square(int(to) - dir)
	...
	case board.MoveCastling:
		...
		// Move the rook
		switch to {
		case board.Square(6): // g1
			rook := newPos.Board.Check(board.Square(7))
			newPos.Board.Clear(board.Square(7))
			newPos.Board.Set(board.Square(5), rook)
		case board.Square(2): // c1
			rook := newPos.Board.Check(board.Square(0))
			newPos.Board.Clear(board.Square(0))
			newPos.Board.Set(board.Square(3), rook)
		case board.Square(62): // g8
			rook := newPos.Board.Check(board.Square(63))
			newPos.Board.Clear(board.Square(63))
			newPos.Board.Set(board.Square(61), rook)
		case board.Square(58): // c8
			rook := newPos.Board.Check(board.Square(56))
			newPos.Board.Clear(board.Square(56))
			newPos.Board.Set(board.Square(59), rook)
		}
	default:
		newPos.Board.Set(to, piece)
	}
	...
	if piece.Type() == board.King {
		if pos.ActiveColor == board.White {
			newPos.Castling &^= WhiteKingside | WhiteQueenside
		} else {
			newPos.Castling &^= BlackKingside | BlackQueenside
		}
	}
	// Rook moves or captures
	switch from {
	case board.Square(0):
		newPos.Castling &^= WhiteQueenside
	case board.Square(7):
		newPos.Castling &^= WhiteKingside
	case board.Square(56):
		newPos.Castling &^= BlackQueenside
	case board.Square(63):
		newPos.Castling &^= BlackKingside
	}
	switch to {
	case board.Square(0):
		newPos.Castling &^= WhiteQueenside
	...
	if piece.Type() == board.Pawn {
		...
		if diff == 16 || diff == -16 {
			newPos.EnPassant = board.Square((int(from) + int(to)) / 2)
		}
		newPos.Halfmoves = 0
	}
	// Reset halfmove on captures
	if pos.Board.Check(to) != board.None {
		newPos.Halfmoves = 0
	}
...
func copyBoard(b *board.Chessboard) *board.Chessboard {
	nb := board.NewChessboard()
...
var WhiteKingside = game.WhiteKingside
var WhiteQueenside = game.WhiteQueenside
var BlackKingside = game.BlackKingside
var BlackQueenside = game.BlackQueenside

func printMoves(t *testing.T, ml board.MoveList) {
	t.Helper()
	for i := 0; i < ml.Count(); i++ {
		t.Logf("  %s", ml.Get(i))
	}
